...
\subsection{microFrameWork} \label{subsec:il-micro-framework}
  ...
  \subsubsection*{I dati}
    ...
    \paragraph{Tag 1: Managed vs Complex}
      ...
      streamer, mentre per quelli Complex ci sono due scenari possibili in
      base al valore del terzo tag:
    \begin{itemize}
      \item Global: l'utente che dichiara il tipo deve implementare un
      costruttore con la segnatura
      \mintinline{cpp}{mycomplex(const ufw::config&)}.
      La \texttt{ownership} delle istanze del tipo in questione appartiene
      al framework;

      \item Context: all'utente che dichiara il tipo è richiesto di creare
      anche la specializzazione esplicita della classe template
      \mintinline{cpp}{ufw::data::factory<mycomplex>} che sulla
      base della configurazione indicata nel json andrà a leggere il file di
      input e a costruire un'istanza del tipo in questione per ogni
      \texttt{context\_id} elaborato, assumendone la \texttt{ownership}.
      \end{itemize}
